...
class ToTensor(object):
    ...
    def __call__(self, points):
        ...
        # Since it is tricky to decide whether to copy data, we leave it to dataset_prep.
...
class RotatePerturbation(object):
    ...
    def __init__(self, angle_sigma=0.06, angle_clip=0.18):
        self.angle_sigma = angle_sigma # 2*np.pi #
        self.angle_clip = angle_clip # 2*np.pi #
        self.axes = np.eye(3)
    ...
    def _get_rotation_matrix(self):
        ...
        angles = torch.clamp(self.angle_sigma * torch.rand(3), -self.angle_clip, self.angle_clip).numpy()
        #angles = torch.clamp(self.angle_sigma * torch.rand(3), 0, self.angle_clip).numpy()
