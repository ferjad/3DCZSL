...
class BoxQuery(torch.autograd.Function):
    @staticmethod
    def forward(ctx, points, centroids, lens, num_neighbours):
        """Ball query
        ...
        """
        index, count = pn2_ext.box_query(points, centroids, lens, num_neighbours)
        return index, count
...
box_query = BoxQuery.apply

class KNNDistance(torch.autograd.Function):
    @staticmethod
    def forward(ctx, query_xyz, key_xyz, num_neighbors, transpose=True):
...
knn_distance = KNNDistance.apply
