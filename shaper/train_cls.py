...
def parse_args():
    ...
    parser.add_argument(
        '--cfg',
        dest='config_file',
        default='/home/evinpinar/Documents/zeroshot3d/compositional_3d/configs/pn_cls.yaml',
        metavar='FILE',
...
def validate(model,
    ...
    iou_metric = SegIoU(num_classes=cfg.DATASET.NUM_SEG_CLASSES)
    ...
    with torch.no_grad():
        for iteration, data_batch in enumerate(dataloader):
            ...
            iou_metric.update_dict(preds, data_batch)

            if iteration%300==0:
                print("Iteration: ", iteration,"IoU: ", iou_metric.value())
        ...
        print("Val split mIOU: {}".format(iou_metric.value()))
    return meters
...
def train(cfg, zero_shot=False, output_dir=''):
    ...
    set_random_seed(cfg.RNG_SEED)
    train_dataloader = build_dataloader(cfg, mode='train', zero_shot=zero_shot, cache_mode=cfg.DATALOADER.KWARGS.cache_mode)
    val_period = cfg.TRAIN.VAL_PERIOD
    val_dataloader = build_dataloader(cfg, mode='val', zero_shot=zero_shot, cache_mode=cfg.DATALOADER.KWARGS.cache_mode) if val_period > 0 else None
